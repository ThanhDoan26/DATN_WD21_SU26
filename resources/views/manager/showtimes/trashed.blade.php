...
@section('title', 'Thùng Rác Suất Chiếu')
            ...
            <tbody>
                @forelse($showtimes as $showtime)
                    <tr>
                        ...
                        <td>
                            <form action="{{ route('manager.showtimes.restore', $showtime->id) }}" method="POST" class="d-inline">
                                @csrf
                                <button class="btn btn-sm btn-success" type="submit" title="Khôi phục">
                                    <i class="fas fa-undo"></i>
                                </button>
                            </form>
                            <form action="{{ route('manager.showtimes.forceDelete', $showtime->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Bạn có chắc muốn xóa vĩnh viễn suất chiếu này?');">
                                @csrf
                                @method('DELETE')
