    ...
    <div class="card-body">
        <form action="{{ route('admin.movies.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="col-md-8">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('title') is-invalid @enderror" 
                               ...
                               placeholder="Ví dụ: Avatar: Dòng Chảy Của Nước" required>
                ...
                <div class="col-md-4">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required onchange="handleStatusChange()">
                            <option value="SCHEDULED" {{ old('status') == 'SCHEDULED' ? 'selected' : '' }}>📅 Lên lịch (Scheduled)</option>
                            <option value="PRE_ORDER" {{ old('status') == 'PRE_ORDER' ? 'selected' : '' }}>🎟️ Mở bán sớm (Pre-order)</option>
                            <option value="COMING_SOON" {{ old('status', 'COMING_SOON') == 'COMING_SOON' ? 'selected' : '' }}>⏳ Sắp chiếu (Coming Soon)</option>
                            <option value="NOW_SHOWING" {{ old('status') == 'NOW_SHOWING' ? 'selected' : '' }}>🎬 Đang chiếu (Now Showing)</option>
                            <option value="ENDED" {{ old('status') == 'ENDED' ? 'selected' : '' }}>🛑 Ngưng chiếu (Ended)</option>
                        </select>
            ...
            <div class="text-end mt-4">
                ...
                <button type="submit" class="btn btn-primary" id="btn-save-movie">Lưu bộ phim</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    // Auto-preview on page load if value exists & setup validation
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const form = document.querySelector('form[action="{{ route('admin.movies.store') }}"]');
        if (form) {
