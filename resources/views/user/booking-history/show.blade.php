            ...
            <div class="p-8 md:p-10">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10 pb-10 border-b border-slate-700/50">
                    ...
                    <div class="flex items-center gap-4">
                        @php
                            $statusIcons = [
                                \App\Models\Booking::STATUS_PAID => 'fa-check-circle text-emerald-500',
                                \App\Models\Booking::STATUS_PENDING => 'fa-hourglass-half text-amber-500',
                                \App\Models\Booking::STATUS_CANCELLED => 'fa-times-circle text-rose-500',
                                \App\Models\Booking::STATUS_EXPIRED => 'fa-clock text-slate-400',
                                \App\Models\Booking::STATUS_USED => 'fa-user-check text-blue-500',
                        ...
                        <div class="w-12 h-12 rounded-2xl bg-slate-700/50 flex items-center justify-center text-xl">
                            <i class="fas {{ $statusIcons[$normalizedStatus] ?? 'fa-info-circle text-slate-400' }}"></i>
                        </div>
                        <div>
                            <p class="text-slate-400 text-xs uppercase font-bold tracking-wider">Trạng thái</p>
                            <p class="text-white text-xl font-bold">{{ $booking->status_label }}</p>
                ...
                <div class="flex flex-col md:flex-row gap-10">
                    ...
                    <div class="md:w-64 flex flex-col items-center">
                        @php
                            $isExpired = $booking->isPaid() && ($booking->showtime->status === \App\Models\Showtime::STATUS_COMPLETED || ($booking->showtime->end_time && $booking->showtime->end_time->isPast()));
                            $ticketQrPayload = $booking->ticket_token ? url('/tickets/' . $booking->ticket_token) : url('/tickets/' . $booking->booking_code);
                        @endphp
                        ...
                        <div class="p-4 bg-white rounded-3xl shadow-2xl mb-4 group/qr">
                            ...
                            @elseif($booking->isPaid())
                                <div class="w-48 h-48 bg-slate-100 rounded-2xl flex items-center justify-center border-4 border-slate-50 overflow-hidden group-hover/qr:scale-105 transition-transform duration-500">
                                     {!! \SimpleSoftwareIO\QrCode\Facades\QrCode::size(160)->margin(0)->generate($ticketQrPayload) !!}
                                </div>
